...
def register(msg, args = None):
    ...
    if args is None or len(args.split()) < 2:
        msg.origin.notice(msg.params, "Syntax: /msg %s \x02REGISTER\x02 <password> <email>" % (var.bots[msg.params].data['nick']))
        return False
    ...
    if user['data']['acname'] == username:
        msg.origin.notice(msg.params, "You are now registered and logged in as \x02%s\x02." % (username))
...
def login(msg, args = None):
    ...
    if args is None or len(args.split()) < 2:
        msg.notice(msg.params, "Syntax: /msg %s \x02LOGIN\x02 <accountname> <password>" % (var.bots[msg.params].data['nick']))
        return False
    if var.users[str(msg.origin)]['account'] != '*':
        msg.origin.notice(msg.params, "You are already logged in as \x02%s\x02." % (var.users[str(msg.origin)]['account']))
        return False
    ...
    if username not in database.__refero__()['users']:
        msg.origin.notice(msg.params, "Username \x02%s\x02 has not been registered yet." % (username))
        return False
    ...
    if valid is False:
        ...
        msg.origin.notice(msg.params, "Login for \x02%s\x02 failed and has been logged." % (username))
        ...
        msg.origin.login(username)
        msg.origin.notice(msg.params, "You are now logged in as \x02%s\x02." % (username))
        if username in var.failed_logins:
            msg.origin.notice(msg.params, "There has/have been %s failed logins since your last login." % (len(var.failed_logins[username])))
            [msg.origin.notice(msg.params, " - Attempt from \x02%s\x02" % (host)) for host in var.failed_logins[username]]
...
def identify(msg, args = None):
    ...
    if args is None or len(args.split()) < 1:
        msg.notice(msg.params, "Syntax: /msg %s \x02IDENTIFY\x02 <password>" % (var.bots[msg.params].data['nick']))
        return False
    if var.users[str(msg.origin)]['account'] != '*':
        msg.origin.notice(msg.params, "You are already logged in as \x02%s\x02." % (var.users[str(msg.origin)]['account']))
        return False
...
def logout(msg):
    ...
    msg.origin.notice(msg.params, "You have been logged out of the account \x02%s\x02." % (var.users[str(msg.origin)]['account']))
    msg.origin.logout()
    return False
...
def group(msg, args = None):
    ...
    except: msg.notice(msg.params, "Syntax: /msg %s \x02GROUP\x02 <nick>" % (var.bots[msg.params].data['nick']))
    if not msg.origin.logged_in or var.users[str(msg.origin)]['account'] == '*':
        msg.origin.notice(msg.params, "You are not currently logged in.")
        return False
    if nick in var.database.__refero__()['users']['grouped']:
        msg.origin.notice(msg.params, "%s is already grouped to another account." % (nick))
        return False
...
def ungroup(msg, args = None):
    ...
    try: 
        nick = args.split()[0]
        account = var.users[str(msg.origin)]['account']
    except: msg.notice(msg.params, "Syntax: /msg %s \x02UNGROUP\x02 <nick>" % (var.bots[msg.params].data['nick']))
    if not msg.origin.logged_in or var.users[str(msg.origin)]['account'] == '*':
        msg.origin.notice(msg.params, "You are not currently logged in.")
        return False
    if nick in var.database.__refero__()['users']['grouped'] and nick not in var.database.__refero__()['users'][account]['grouped']:
        msg.origin.notice(msg.params, "%s is not owned by you and is grouped to another account." % (nick))
        return False
    if nick not in var.database.__refero__()['users']['grouped'] and nick not in var.database.__refero__()['users'][account]['grouped']:
        msg.origin.notice(msg.params, "%s is not grouped to any account, and therefore cannot be removed." % (nick))
        return False
